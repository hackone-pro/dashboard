...
function getStatusByTimestamp(timestamp: string | null) {
    ...
    if (diffMinutes <= 59) return "🟢";        // verde até 59m
    if (diffMinutes <= 119) return "🟡";       // amarelo até 1h59
    return "🔴";                                // vermelho após 2h
}
...
const FirewallCard = forwardRef<FirewallCardRef>((props, ref) => {
    ...
    async function carregar() {
        try {
            ...
            // 🔵 Antes tinha só isso (e por isso os logs sumiram)
            // const logs = await getTopFirewalls("10min");
    
            // 🔥 Agora voltamos com o que funcionava antes:
            const logs10 = await getTopFirewalls("10min");   // logs recentes
            const logsLast = await getTopFirewalls("todos"); // fallback histórico
    ...
    // expõe método carregar() para o pai
                    ...
                    <table className="w-full text-xs text-gray-400">
                        <thead className="fundo-dashboard">
                            <tr className="text-white">
                                ...
                                <th className="text-left py-2">Últimos Logs</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            {firewallsPaginados.map((fw, index) => (
                                ...
                                >
                                    ...
                                    <td>{fw.timestamp ? new Date(fw.timestamp).toLocaleString() : "-"}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
