...
export default function Incidentes() {
    ...
    // Remove "#1098 - " e "[hh:mm - dd/mm/aaaa] - (Baixo|Médio|Alto|Crítico) - " do título
    function formatCaseName(name: string) {
        return name
            .replace(/^\s*#\s*\d+\s*-\s*/i, "")
            .replace(
                /\[\s*\d{2}:\d{2}\s*-\s*\d{2}\/\d{2}\/\d{4}\s*\]\s*-\s*(Baixo|Baixa|M[eé]dio|M[eé]dia|Alto|Alta|Cr[ií]tico|Cr[ií]tica)\s*-\s*/i,
                ""
            )
            .trim();
    }
