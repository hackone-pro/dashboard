# Cronograma — Fase 1 (MVP Funcional)
...
| Incluido | Excluido |
|----------|----------|
| E01-E07, C01-C02, A01, R01-R04, T01 (todo o rascunho) | G05 (merge multi-fonte) |
...
BLOCO 5 — Frontend Essentials        BLOCO 6 — Validacao E2E
(depende dos Blocos 2-4)            (depende de tudo)

  [R03]+[E07]──► [G10] Risk Level     [pipeline funcional]
                  cards dinamicos          │
  [E07]────────► [G11] Monitoria       ┌──▼──┐
                  Ingestao             │ A01 │ SOC Analytics
                                       └─────┘
```
...
| 6 | R01 | Documentar regras do Risk Level | Documentacao | 1 | — | | |
| 7 | C01 | Separar config LLM (Chat vs Motor) | Full Stack | 1 | — | | |
| 8 | E04 | Propagar tenant_id nos microservicos | Back-end | 2 | E05 | | |
| 9 | G01 | Adapter pattern FortiGATE (Collector + Normalizer) | Back-end | 2 | E03 | | |
| 10 | G03 | Deduplicacao de eventos no ALERT | Back-end | 2 | E03 | | |
| 11 | E07 | Feature flags por plano do tenant | Front-end | 2 | E06 | | |
| 12 | R02 | Auditar Risk Level atual vs especificacao | Back-end | 2 | R01 | | |
| 13 | C02 | Configurar LLM padrao Hackone como fallback | Back-end | 2 | C01 | | |
| 14 | G02 | Mapeamento de severidade FortiGATE | Back-end | 3 | G01 | | |
| 15 | E01 | Tela de configuracao de fontes FortiGATE | Front-end | 3 | E06 | | |
| 16 | E02 | Background job de polling FortiGATE | Back-end | 3 | E01, E03, G01 | | |
| 17 | R03 | Corrigir Risk Level (degradacao graciosa) | Back-end | 3 | R02 | | |
| 18 | G04 | Motor de correlacao (Incident Engine) | Back-end | 4 | E03, G01, G03, G06 | | |
| 19 | G17 | Calculo de severidade final do incidente | Back-end | 4 | G04 | | |
| 20 | G15 | Classificacao de ativos criticos (crown jewel) | Full Stack | 4 | G04 | | |
| 21 | G16 | Fallback de IA (Hackone como padrao) | Back-end | 4 | C01, G04 | | |
| 22 | R04 | Testes Risk Level (3 cenarios) | Back-end | 4 | R03 | | |
| 23 | G10 | Risk Level com cards dinamicos (front) | Front-end | 5 | R03, E07 | | |
| 24 | G11 | Monitoria de Ingestao no Essentials | Front-end | 5 | E07 | | |
| 25 | A01 | Validar SOC Analytics com dados Essentials | Full Stack | 6 | E02, G04 | | |
...
Back-end 1:    E03 ──────────► G01 ──────────► G02 ──► E02 ───► G04 ──► G17                   
               (ALERT core)    (adapter FG)    (sev)   (job)    (correlation)                  

Back-end 2:    E06 ► E05 ────► E04              R03 ──────────► G15 ──► G16 ──────────────────► A01
               (tenant/JWT)    (tenant_id)      (risk fix)       (crown)  (fallback)

Back-end 3:    G06 ► T01 ────► G03 ──────────► R02 (audit) ───► R04 (testes)
               (taxonomia)     (dedup)

Full Stack:    C01 ──────────► C02
               (LLM config)    (LLM default)

Front-end:                     E07 ──► E01 ────────────────────────────────────────► G10 ► G11
                               (flags)  (tela FG)                                   (risk) (monit)
```
...
### BLOCO 6 — Validacao End-to-End

---

#### A01 — Validar SOC Analytics com dados do Essentials

**Titulo:** Testar e ajustar metricas do SOC Analytics (MTTD/MTTA/MTTR) com dados reais do pipeline Essentials

**Descricao:**
Com o pipeline funcional (FortiGATE → ALERT → CORRELATION → TICKET → IRIS), validar se as metricas de performance do SOC sao calculadas corretamente com dados que nao vem de SIEM.

**O que fazer:**
- Gerar fluxo real: configurar FortiGATE (ou mock), disparar polling, verificar que incidentes sao criados no IRIS
- Verificar no SOC Analytics (`/soc-analytics`):
  - **MTTD** (Mean Time To Detect): diferenca entre `event_time` do primeiro evento e `ingest_time` no ALERT. No Essentials, limitado ao intervalo de polling (5-15 min)
  - **MTTA** (Mean Time To Acknowledge): diferenca entre `created_at` do incidente e `acknowledged_at` (quando alguem atribui)
  - **MTTR** (Mean Time To Resolve): diferenca entre `created_at` e `resolved_at`
  - **Incidentes abertos:** contagem correta
- Se alguma metrica nao calcula corretamente:
  - Ajustar queries no backend (`api-hackone-tickets` — endpoint `/api/analytics/soc`)
  - Ajustar componentes no frontend se necessario
- Documentar limitacoes do Essentials (ex: MTTD limitado pelo intervalo de polling)

**Criterios de aceite:**
- [ ] MTTD calculado corretamente com timestamps do Essentials
- [ ] MTTA funcional quando incidente e atribuido
- [ ] MTTR funcional quando incidente e fechado
- [ ] Contagem de incidentes abertos correta
- [ ] Limitacoes documentadas
- [ ] Pagina SOC Analytics renderiza sem erros para tenant `essentials`

**Tipo:** Full Stack
**Servicos afetados:** API Tickets — `/api/analytics/soc` | Frontend — `pages/SOCAnalytics.tsx`
...
| 5 — Frontend Essentials | G10, G11 | Blocos 2-4 |
| 6 — Validacao E2E | A01 | Blocos 1-5 |
